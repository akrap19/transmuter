...
describe("queryCoins", () => {
  ...
  it("filters by backing asset", () => {
    const mixed = [...catalog, coin({ mint: "MintBtc11111111111111111111111111111111111", name: "Forge", symbol: "FRG", status: "sale", backing: "cBTC", launchedAt: 400 })];
  ...
  it("sorts by market cap with nulls last, and by name ascending", () => {
    expect(queryCoins(catalog, { sort: "marketCapUsd", dir: "desc" }).items.map((item) => item.symbol)).toEqual(["HLX", "AUR", "NMB"]);
